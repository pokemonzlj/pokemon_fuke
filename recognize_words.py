...
class fuke(contrast_pic):
    ...
    def get_pic(self,id='1163746998'):
        ...
        subprocess.Popen('adb  -s %s shell screencap -p /sdcard/DCIM/screenshot.png' %(self.device_id) ).wait()  #掉出宝箱之后截图以便查看
        subprocess.Popen('adb  -s %s pull /sdcard/DCIM/screenshot.png %s/%s.png ' %(self.device_id,path,timepic),stdout=subprocess.PIPE).wait()
    ...
    def check_package(self,id='1163746998'):
        r=os.popen("adb -s %s shell dumpsys window | findstr mCurrentFocus" %id,"r")  #想获取控制台输出的内容，那就用os.popen的方法了，popen返回的是一个file对象，跟open打开文件一样操作了，r是以读的方式打开
    ...
    def read_word(self,weizhi='up'):
        ...
        #     bili=1
        if weizhi=='up':
            pic.crop((int(905*self.bili+self.extra_distance),526,int(1050*self.bili+self.extra_distance),610)).save(cut_pic_path)  #适用于找到 匹配中 3个字
            ...
            pic.crop((int(840*self.bili+self.extra_distance),950,int(950*self.bili+self.extra_distance),960)).save(cut_pic_path)  #适用于找到 在线匹配 4个字框的下面的左半边部分
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            ...
            for y in range(pic_new.size[1]):  # 二值化处理，这个阈值为R=95，G=95，B=95
                for x in range(pic_new.size[0]):   #size[0]即图片长度，size[1]即图片高度
                    if pix[x, y][0] ==255 and 198<= pix[x, y][1] <= 202 and pix[x, y][2] <=10:
                        return 'battle'
                    elif 22<= pix[x, y][0] <=30 and 16<= pix[x, y][1] <= 24 and  pix[x, y][2] <=5:   #有宝箱界面遮挡
                        return 'valuebox'
            return ''
        elif weizhi=='level':
            pic.crop((int(890*self.bili+self.extra_distance),905,int(1020*self.bili+self.extra_distance),980)).save(cut_pic_path)  #适用于找到 确定 2个字
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            for y in range(pic_new.size[1]):  # 二值化处理，这个阈值为R=95，G=95，B=95
            ...
            pic.crop((int(870*self.bili+self.extra_distance),570,int(1050*self.bili+self.extra_distance),630)).save(cut_pic_path)  #适用于找到 （对方已经逃跑）我知道啦 4个字
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            for y in range(pic_new.size[1]):  # 二值化处理，这个阈值为R=95，G=95，B=95
            ...
            pic.crop((int(1333*self.bili+self.extra_distance),637,int(1426*self.bili+self.extra_distance),690)).save(cut_pic_path)  #适用于找到 右下角的蓝旗
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            # pic_new=pic_new.convert('1')  #convert方法可以将图片转成黑白
            ...
            pic.crop((int(576*self.bili+self.extra_distance),637,int(677*self.bili+self.extra_distance),690)).save(cut_pic_path)  #适用于找到 左下角的蓝旗
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            # pic_new=pic_new.convert('1')  #convert方法可以将图片转成黑白
            ...
            pic.crop((int(957*self.bili+self.extra_distance),400,int(1054*self.bili+self.extra_distance),452)).save(cut_pic_path)  #适用于找到 顶端的蓝旗
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            # pic_new=pic_new.convert('1')  #convert方法可以将图片转成黑白
            ...
            if word != "" and word !=None and word.isdigit():
                ...
                print "Can not read the zhanli, refind it!"
                self.get_screenshot()
                self.read_word('qiangkuang_zhanli')
        elif weizhi=='qiangkuang_jiangli':
            pic.crop((int(872*self.bili+self.extra_distance),862,int(1045*self.bili+self.extra_distance),878)).save(cut_pic_path)  #适用于找到 打完整个抢矿后弹出的奖励界面的 确定 图标,为了跟战斗的确定区分开，只取图标的上面一小块
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            # pic_new=pic_new.convert('1')  #convert方法可以将图片转成黑白
            ...
            pic.crop((int(1745*self.bili+self.extra_distance),50,int(1810*self.bili+self.extra_distance),70)).save(cut_pic_path)  #适用于找到 切换自动战斗的咖啡色小图标
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            # pic_new=pic_new.convert('1')  #convert方法可以将图片转成黑白
            ...
            pic.crop((int(1570*self.bili+self.extra_distance),271,int(1700*self.bili+self.extra_distance),283)).save(cut_pic_path)  #适用于找到 出战精灵队伍选择的确定框
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            # pic_new=pic_new.convert('1')  #convert方法可以将图片转成黑白
            ...
            pic.crop((int(870*self.bili+self.extra_distance),755,int(1045*self.bili+self.extra_distance),770)).save(cut_pic_path)
            pic_new=Image.open(cut_pic_path)
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            # pic_new=pic_new.convert('1')  #convert方法可以将图片转成黑白
            ...
            pic_new=pic_new.convert('RGBA')
            pix=pic_new.load()
            for x in range(int(1210*self.bili+self.extra_distance),int(1450*self.bili+self.extra_distance)):   #size[0]即图片长度，size[1]即图片高度
    ...
    def qiangkuang_battle(self,power=400000):
        ...
        if zhanli< power:  #只打战力小于设定值power的对手
            ...
            self.click(int(1327*self.bili+self.extra_distance),343)  #点击抢夺
            self.delay(5)
            for i in range(5):
                self.get_screenshot('pic')
            ...
            self.click(int(1475*self.bili+self.extra_distance),260)   #点击退出
            return False
    ...
    def qiangkuang(self,power=400000):
        ...
        for i in range(300):  #只抢3人防守的矿
            ...
            if judge==False:
                ...
                self.get_screenshot('pic')
                self.cut_pic((int(1306*self.bili+self.extra_distance),325),(int(1454*self.bili+self.extra_distance),495))  #抢矿的大图标
    ...
    def get(self):
        if self.check_package()==True:
            ...
            bili=1
        self.get_screenshot()
        self.cut_pic((int(957*self.bili+self.extra_distance),400),(int(1054*self.bili+self.extra_distance),452))
        ...
        for a in range(5):  #一次性将所有的图片都识别成对应编号
            for b in range(12):
                ...
                if self.contrast_two_pic('original_pic','lianlian_baoyinguai','target_pic')==True:
                    ...
                    pic_location[a][b]=2
                elif self.contrast_two_pic('original_pic','lianlian_dajia','target_pic')==True or self.contrast_two_pic('original_pic','lianlian_dajia2','target_pic')==True:
                    pic_location[a][b]=3
                    ...
                    pic_location[a][b]=5
                elif self.contrast_two_pic('original_pic','lianlian_dazhenfeng','target_pic')==True or self.contrast_two_pic('original_pic','lianlian_dazhenfeng2','target_pic')==True:
                    pic_location[a][b]=6
                elif self.contrast_two_pic('original_pic','lianlian_dazuiou','target_pic')==True:
                    pic_location[a][b]=7
                elif self.contrast_two_pic('original_pic','lianlian_dazuiwa','target_pic')==True or self.contrast_two_pic('original_pic','lianlian_dazuiwa2','target_pic')==True:
                    pic_location[a][b]=8
                elif self.contrast_two_pic('original_pic','lianlian_shamonaiya','target_pic')==True or self.contrast_two_pic('original_pic','lianlian_shamonaiya2','target_pic')==True:
                    pic_location[a][b]=9
                elif self.contrast_two_pic('original_pic','lianlian_zhichongxiong','target_pic')==True or self.contrast_two_pic('original_pic','lianlian_zhichongxiong2','target_pic')==True:
                    pic_location[a][b]=10
                else:
                    pic_location[a][b]=0
        print pic_location
    ...
    def lianliankan(self):
        ...
        thread.start_new_thread(self.lianlian_shibie,(pic_location,))  #将图片识别放入线程执行
        ...
        while total_count>0:
                                    ...
                                    if pic[i][j]==pic[down][right]:
                                        ...
                                        for forward in range(j+1,right):
                                            ...
                                            if pic[i][j]==pic[goup][forward]:
                                                ...
                                                self.click(x1,y1,True)
                                                self.click(x2,y2,True)
                                                print("Xiang xia you de shang xia liang mian Xiaochu (%s,%s) (%s,%s)"%(i,j,goup,forward))
                                                pic[i][j]=0
                                                pic[goup][forward]=0
                                                total_count-=2
                                                continue
            ...
            if last_total_count>total_count+2:  #这一轮消除掉超过一对
                ...
                thread.start_new_thread(self.lianlian_shibie,(pic_location_new,))  #将图片识别放入线程执行
    ...
    def select_device(self):
        ...
        if devicenum==0:
            ...
            for i in range(devicenum):
                string=subprocess.Popen("adb -s %s shell getprop ro.product.device" %devicelist[i],shell=True,stdout=subprocess.PIPE)
    ...
    def get_ballery_level(self,deviceid=''):
        battery_info=subprocess.Popen("adb -s %s shell dumpsys battery" %deviceid,shell=True,stdout=subprocess.PIPE)
    ...
    def start_play(self):
        ...
        string=subprocess.Popen("adb -s %s shell getprop ro.product.model" %self.device_id,shell=True,stdout=subprocess.PIPE)
        self.device_model=string.stdout.read().strip()  #去除掉自动生成的回车
        # print "Input the num to choose device type:\n 1 T1 \n 2 NOT T1"
        # dev=input()
        if "T7" in self.device_model:
